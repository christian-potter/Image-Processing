...
headers = {'<Timelapse','<LSM name','<PMT enable','<Streaming enable','<ZStage name'};
headidx= nan(length(headers),1); 

%'<Pockels type=1'(ignored header); 
...
header{4} = {'flybackTimeAdjustMS','flybackLines','flybackFrames'};
header{5} = {'steps','stepSizeUM','startPos','setupPositionMM'}; 

%% CONVERT TO NAMES THAT WILL BE USED IN STRUCTURE 

values= {'timepoints','pixelsize','width_mm','height_mm','xpix','ypix','dwellTime','averageNum','green_gain','red_gain','flybackTime','flybackLines','flybackFrames','nplanes','stepSize','startPos','setupPosition'};

%%
count = 0; 
for i = 1:length(header)
    
    if ~isnan(headidx(i)) % for when the code doesn't find the header,
