function [] = xyz_location(xyz, ypix_zplane,depth,Fall,zlocs)
arguments
...
%% NOTES
% need to add depth surface plotting 
...
for i = 1:length(ypix_zplane)
    ...
    coords = [1 1 zlocs(curplane(1)) ; ref_yx(2) 1 zlocs(curplane(1)); 1 ref_yx(1) zlocs(curplane(end)); ref_yx(2) ref_yx(1) zlocs(curplane(end))];
    adj = depth.surfaceMapSmooth;
    adj_coords = coords; 
